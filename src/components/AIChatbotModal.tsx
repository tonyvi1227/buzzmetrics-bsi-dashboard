...
export const AIChatbotModal: React.FC<AIChatbotModalProps> = ({
  ...
  const handleStep1Submit = (e: React.FormEvent) => {
    ...
    if (!categoryInterest.trim()) {
      setError('Please enter your industry/category of interest.');
      return;
    }
    setError('');
    setStep(2);
  ...
  const handleFinalLeadSubmit = async (e: React.FormEvent) => {
    ...
    const aiSummary = `Industry: ${categoryInterest}. Brand: ${brandInterest || 'N/A'}. Need: ${actualNeed}. ${customNeedNote ? `Notes: ${customNeedNote}` : ''}`;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">
                30-second AI qualification to unlock Full Dashboard
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain py-4 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 p-3.5 rounded-2xl rounded-tl-none max-w-[85%] text-xs font-semibold text-slate-800 dark:text-slate-200 space-y-1">
              <p>Hello! I am your Buzzmetrics AI Insight Assistant.</p>
              <p>Please share the **Industry/Category** and **Brand** you are interested in exploring:</p>
            </div>
          </div>
              ...
              <form onSubmit={handleStep1Submit} className="space-y-3 bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Handhelds, Banking, Beverage, E-commerce..."
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">
                    BRAND OF INTEREST (OPTIONAL)
                  </label>
                  <input
                    type="text"
                    value={brandInterest}
                    onChange={(e) => setBrandInterest(e.target.value)}
                    placeholder="e.g. Samsung, Heineken, Vinamilk..."
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-bold rounded-xl p-2.5 text-slate-900 dark:text-white"
                  />
                </div>
            ...
            <>
              <div className="flex justify-end">
                <div className="bg-buzz text-white p-3 rounded-2xl rounded-tr-none text-xs font-bold max-w-[80%] shadow-sm">
                  Category: {categoryInterest} {brandInterest ? `(Brand: ${brandInterest})` : ''}
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 animate-fadeIn">
                ...
                <div className="bg-slate-100 dark:bg-slate-800 p-3.5 rounded-2xl rounded-tl-none max-w-[85%] text-xs font-semibold text-slate-800 dark:text-slate-200 space-y-1">
                  <p>Great! Regarding **{categoryInterest}**.</p>
                  <p>What is your primary analytics objective or business need?</p>
                </div>
              </div>
            </>
          )}
          ...
          {step === 2 && (
            <div className="space-y-2 pl-11 animate-fadeIn">
              {[
                'Explore General Benchmark & Industry Baselines',
                'Preparing Upcoming Campaign / New Product Launch',
                'Competitor Benchmarking (BSI & Sentiment Analysis)',
                'Need Post-Campaign Performance Audit / Report',
              ].map((needOption) => (
                <button
                  key={needOption}
          ...
          {step >= 3 && (
            <>
              ...
              <div className="flex items-start gap-3 animate-fadeIn">
                ...
                <div className="bg-slate-100 dark:bg-slate-800 p-3.5 rounded-2xl rounded-tl-none max-w-[85%] text-xs font-semibold text-slate-800 dark:text-slate-200 space-y-1">
                  <p>Thank you! Please fill in your professional contact details to instantly unlock the Full Dashboard:</p>
                </div>
              </div>
            </>
          )}
            ...
            <div className="pl-11 animate-fadeIn">
              <form onSubmit={handleFinalLeadSubmit} className="space-y-3 bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase">Phone / Mobile (*)</label>
                    <input
                      type="tel"
                      required
                      value={phone}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase">Work Email (*)</label>
                    <input
                      type="email"
                      required
                      value={workEmail}
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase">Company Name (*)</label>
                    <input
                      type="text"
                      required
                      value={company}
                      onChange={(e) => setCompany(e.target.value)}
                      placeholder="Samsung, Unilever..."
                      className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-bold rounded-lg p-2 text-slate-900 dark:text-white"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-500 uppercase">Additional Notes (Optional)</label>
                  <input
                    type="text"
                    value={customNeedNote}
                    onChange={(e) => setCustomNeedNote(e.target.value)}
                    placeholder="Campaign scope, timeline, or specific questions..."
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-bold rounded-lg p-2 text-slate-900 dark:text-white"
                  />
                </div>
                ...
                >
                  ...
                  <span>ACTIVATE AI & UNLOCK FULL DASHBOARD</span>
                </button>
              </form>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="pl-11 animate-fadeIn">
              <div className="p-4 bg-emerald-50 dark:bg-emerald-950/60 rounded-2xl border border-emerald-300 dark:border-emerald-800 text-center text-emerald-700 dark:text-emerald-300 space-y-2">
                ...
                <p className="text-xs font-black">AI VERIFICATION COMPLETE!</p>
                <p className="text-[11px] font-semibold">Unlocking Full Dashboard for you now...</p>
              </div>
            </div>
          )}
        </div>
