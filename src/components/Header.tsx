...
import { Sun, Moon, Upload, Download, RotateCcw, Calendar, Lock } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="glass-card rounded-2xl p-4 md:p-6 mb-6 shadow-sm flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 border-l-4 border-l-buzz">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            <h1 className="text-xl md:text-2xl lg:text-3xl font-black tracking-tight text-slate-900 dark:text-white uppercase leading-tight">
              BUZZMETRICS <span className="text-buzz">BSI CAMPAIGN DASHBOARD</span>
            </h1>
          ...
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-0.5">
            Consumer Insight & Brand Performance Report ({totalRecordsCount} Chiến dịch)
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2.5 w-full xl:w-auto justify-start xl:justify-end">
        ...
        {/* Import Data Button (Requires Admin Lock) */}
        <button
